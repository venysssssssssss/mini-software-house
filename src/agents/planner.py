...
class PlannerAgent(Agent):
    ...
    def plan_task(self, user_request: str) -> dict:
        ...
        # Native JSON Mode (Grammar-Based Sampling) ensures valid JSON output
        ...
        try:
            ...
            # Check if project_name follows kebab-case pattern
            if not self._is_valid_kebab_case(project_name):
                ...
                project_name = engine.generate_project_name_from_task(
                    task_description=user_request, dependencies=data.get("dependencies", [])
                )
    ...
    def _is_valid_kebab_case(self, name: str) -> bool:
        """Validate if name follows kebab-case pattern."""
        if not name:
            return False
